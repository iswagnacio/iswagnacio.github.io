...
def blend_pyramids(lap_pyr1, lap_pyr2, mask_pyr):
    ...
    for l1, l2, m in zip(lap_pyr1, lap_pyr2, mask_pyr):
        ...
        if len(l1.shape) == 3 and len(m_norm.shape) == 2:
            m_norm = m_norm[:, :, np.newaxis]
        
        blended_level = l1 * m_norm + l2 * (1 - m_norm)
        blended.append(blended_level)
    
    return blended

def pyramid_blend(img1, img2, mask1, mask2, levels=4):
    weight1 = distance_transform_edt(mask1 > 0).astype(np.float64)
    weight2 = distance_transform_edt(mask2 > 0).astype(np.float64)
    total_weight = weight1 + weight2
    total_weight[total_weight == 0] = 1
    
    blend_mask = weight1 / total_weight
    lap_pyr1 = build_laplacian_pyramid(img1, levels)
    lap_pyr2 = build_laplacian_pyramid(img2, levels)
    mask_pyr = build_gaussian_pyramid(blend_mask, levels)
...
def blend_two_images_pyramid(img1, img2, H_1to2, accumulator=None, pyramid_levels=4):
    h1, w1 = img1.shape[:2]
    h2, w2 = img2.shape[:2]
    ...
    if accumulator is None:
        ...
        canvas_img2[copy_y:copy_y+old_h, copy_x:copy_x+old_w] = accumulator['image']
        mask2[copy_y:copy_y+old_h, copy_x:copy_x+old_w] = accumulator['mask']

    T = np.array([[1, 0, offset_x], [0, 1, offset_y], [0, 0, 1]])
    H_combined = T @ H_1to2
    warped1, mask1 = warpImageBilinear(img1, H_combined, output_shape=(canvas_h, canvas_w))

    result = pyramid_blend(warped1, canvas_img2, mask1, mask2, levels=pyramid_levels)
...
def create_panorama_sequential_pyramid(images, homographies, pyramid_levels=4):
    n = len(images)
    ref_idx = n // 2
...
def create_panorama_from_data_pyramid(base_path, image_numbers, output_name='panorama', pyramid_levels=4):
    ...
    plt.imshow(panorama)
    plt.axis('off')
    plt.title(f'Panorama: DSC_0{image_numbers[0]} to DSC_0{image_numbers[-1]} (Pyramid Blend)')
    plt.tight_layout()
